...
class ProcessMenu:
    ...
    def add_to_item_dictionary(self, menu_item_index: int, amount: int = 1 ) -> dict: 
        ...
        if name in self.item_dict:
            self.item_dict[name] = (price, self.item_dict[name][1] + amount) # Adds the amount to the existing item in the dictionary.
        else:
    ...
    def calculate_total_cost_with_amount(self, dictionary, current_total) -> float:
        """ Calculates the total cost of all items in the dictionary. """
        for value in dictionary.values():
